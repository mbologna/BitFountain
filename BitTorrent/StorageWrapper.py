...
class StorageWrapper(object):
    ...
    def new_request_DF(self, index):
        request_size = self.config['download_slice_size']
        try:
            begin = indexptr[index]
            begin += request_size
            indexptr[index] = begin
        except KeyError:
            indexptr[index] = self.piece_size
            begin = self.piece_size
        self.numactive[index] += 1
        self.stat_active[index] = 1
        if index not in self.stat_dirty:
            self.stat_new[index] = 1
        return (begin, request_size)
    ...
    def piece_came_in(self, index, begin, piece, source = None):
        ...
        if not self.inactive_requests[index] and not self.numactive[index]:
            ...
            if sha(self.storage.read(self.piece_size * self.places[index], self._piecelen(index))).digest() == self.hashes[index]:
                print '%' * 50 
                print 'Piece %d validated OK' % index
                print '%' * 50 
                ...
                print '%' * 50 
                print 'WARNING: Piece %d validated KO.\nRe-downloading...' % index
                print '%' * 50 
    ...
    def DF_piece_came_in(self, index, begin, piece, source = None):
        if self.places[index] < 0:
            if self.rplaces[index] == ALLOCATED:
                self._initalloc(index, index)
            else:
        ...
        if self._piecelen(index) == self._piecelen(0):
            if self._piecelen(0) == 32768:
                try:
                    ...
                    x = RandomDF(int(self._piecelen(0)/self.config['download_slice_size']),self.config['download_slice_size'])
                    dfwrapper[index] = x
                readyToDecode = x.use_random_block(int(begin/len(piece)),piece)
            else:
                try:
                    x = dfwrapper[index]
                except KeyError:
                    x = Raptor(int(self._piecelen(0)/self.config['download_slice_size']),self.config['download_slice_size'])
                    dfwrapper[index] = x
                readyToDecode = x.decode(int(begin/len(piece)),piece)
            ...
            if readyToDecode == True:
                ...
                self.storage.write(self.places[index] * self.piece_size, x.retrievedata())
                del dfwrapper[index]
                del x
                del self.stat_dirty[index]
                if sha(self.storage.read(self.piece_size * self.places[index], self._piecelen(index))).digest() == self.hashes[index]:
                    print 'DF Piece %d validated OK' % index
                    print '#' * 50 
                    self.have[index] = True
                    self.storage.downloaded(index * self.piece_size,
                                            self._piecelen(index))
                    ...
                    print 'WARNING: DF Piece %d validated KO.\nRe-downloading...' % index
                    print '#' * 50 
                    self.data_flunked(self._piecelen(index), index)
                    self.inactive_requests[index] = 1
                    self.amount_inactive += self._piecelen(index)
                    ...
                    return False
                return True       
            elif not self.inactive_requests[index] and not self.numactive[index]:
                return 'request more'     
        else:
            return self.piece_came_in(index, begin, piece, source)

    def request_lost(self, index, begin, length):
        #if begin/length < self._piecelen(index)/self.config['download_slice_size']:
    ...
    def get_piece(self, index, begin, length):
        ...
        if begin + length > self._piecelen(index):
            return None
        block = self.storage.read(self.piece_size * self.places[index] + begin, length)
        print '\tsending "normal" block'
        return block
    
    def get_DF_piece(self, index, begin, length):
        if not self.have[index]:
            return None
        if not self.waschecked[index]:
            if sha(self.storage.read(self.piece_size * self.places[index], self._piecelen(index))).digest() != self.hashes[index]:
        ...
        if self._piecelen(index) == self._piecelen(0):
            piece = self.storage.read(self.piece_size * self.places[index], self.piece_size)
            if self._piecelen(0) == 32768:
                block = create_random_block(rand50list(int(begin/length),int(self._piecelen(0)/self.config['download_slice_size'])),piece).tostring()
                print '\tsending randomDF block'
            else:
                try:
                    x = dfwrapper_tx[index]
                except KeyError:
                    x = Raptor(int(self._piecelen(0)/self.config['download_slice_size']),self.config['download_slice_size'])
                    dfwrapper_tx[index] = x
                block = x.encode(int(begin/length),piece).tostring()
                print '\tsending raptorDF block'
            return block
        else:
            return self.get_piece(index, begin, length)
