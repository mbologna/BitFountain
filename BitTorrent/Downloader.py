...
class SingleDownload(object):
    ...
    def got_piece(self, index, begin, piece):
        print 'IN  block: (piece %d[%d:%d] - SN: %d)' %(index,begin,begin+len(piece),begin/len(piece))
        try:
        ...
        if 'BF2-0-0' in self.connection.id:
            insert = self.downloader.storage.DF_piece_came_in(index, begin, piece,self.guard)
            if insert == 'request more':
                self._request_DF(index)
        else:
            insert = self.downloader.storage.piece_came_in(index, begin, piece,self.guard)
        if not insert:
            if self.downloader.storage.endgame:
    ...
    def _request_DF(self, interest):
        begin, length = self.downloader.storage.new_request_DF(interest)
